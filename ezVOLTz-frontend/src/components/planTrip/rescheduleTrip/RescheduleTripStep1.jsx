...
const RescheduleTripStep1 = ({
  ...
  const getDirections = async ({ origin, destination, previousStep, step }) => {
    ...
    if (formValues?.startTime && formValues?.startTime <= new Date())
      return toast.error('Trip start date and time must be the future date.');
...
export default RescheduleTripStep1;
