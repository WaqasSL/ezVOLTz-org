...
import { toogleVehicleManufactures } from 'redux/dashbbaord/dashboardSlice';
...
export default function AddVehicle({
  ...
  const handleImagePreview = (file) => {
    if (!file.type.startsWith('image/'))
      return toast.error(
        `Image ${file.type} is not supported, please upload an image.`
      );
    setFile(file);
  ...
  const handleReset = () => {
    ...
    setFile(null);
    setManufactureList(dashboard?.vehicleManufactures || []);
    setModels([]);
  };
  ...
  const getVehicleManufacture = async () => {
    ...
    if (result?.status === 200)
      dispatch(toogleVehicleManufactures(result?.data?.vehicleManufactur));
  };
  ...
  const handleAddVehicles = async () => {
    ...
    data.append('userId', auth?.userInfo?.user?._id);
  ...
  return (
    <Transition.Root show={isVehicleModal || false} as={Fragment}>
      <Dialog
        as='div'
        className='relative z-10'
        initialFocus={cancelButtonRef}
        onClose={() => setIsVehicleModal(true)}
      >
        <Transition.Child
          as={Fragment}
                  ...
                  <div className='ez__Form w-full grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4 py-16 px-8'>
                    <div className='ez__VehicleImage block w-full'>
                      {isImagePreview?.isPreview ? (
                        ...
                        <FileUploader
                          ...
                          name='file'
                          maxSize={10}
                          label='Upload or drap & drop a image here'
                        />
                      )}
                    </div>
                    <div className='block w-full'>
                      ...
                      <input
                        ...
                        className='block w-full mb-4 border border-ezGray py-3 text-base text-ezGreen px-3 rounded-md'
                      />
                      <div className='block'>
                        <button
                          type='submit'
